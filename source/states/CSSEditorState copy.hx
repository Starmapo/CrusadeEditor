...
class CSSEditorState extends FlxState
{
	override public function create()
	{
		...
		_slotSelect.alpha = 0.0001;
		_slotSelect.active = false;
		_slotSelect.scrollFactor.set();
		add(_slotSelect);

		_selectedMugshot = new FlxSprite();
		_selectedMugshot.visible = false;
		_selectedMugshot.alpha = 0.8;
		_selectedMugshot.active = false;
		_selectedMugshot.scrollFactor.set();
		add(_selectedMugshot);
	...
	override public function update(elapsed:Float)
	{
		...
		var overlapSlot:CharacterSlot = null;
		for (i in 0..._css.length)
		{
			var slot = _css.members[i];
			for (camera in slot.cameras)
			{
				if (slot.overlapsPoint(FlxG.mouse.getWorldPosition(camera, _point), true, camera))
				{
					...
					if (FlxG.mouse.justPressed && _clickTimer <= 0)
					{
						if (!_contextMenu.visible)
						{
							_selectedMugshot.loadGraphicFromSprite(slot.mugshot);
							_selectedMugshot.setPosition(slot.mugshot.x, slot.mugshot.y);
							_selectedMugshot.scale.copyFrom(slot.mugshot.scale);
							_selectedMugshot.updateHitbox();
							_selectedMugshot.visible = true;
							_selectedID = slot.ID;
							_startMousePos.copyFrom(_point);
							_startMugshotPos.set(_selectedMugshot.x, _selectedMugshot.y);
						}
						_contextMenu.visible = false;
					}
		...
		{
			_slotSelect.alpha = 0.00001;
		}
		if (overlapSlot != null && overlapSlot.char.length > 0 && overlapSlot.char != 'random')
		{
			if (_lastCharText != overlapSlot.char)
			{
				_updateCharText(CharacterData.characters.get(overlapSlot.char));
			}
		}
		else
		{
		...
		if (FlxG.mouse.justReleased && _selectedMugshot.visible)
		{
			_selectedMugshot.visible = false;
			if (overlapSlot != null)
			{
				...
				{
					_css.swapIcons(_selectedID, overlapSlot.ID);
				}
				else
				{
					_css.moveIcon(_selectedID, overlapSlot.ID);
				}
			}
			else
			{
				_css.clearSlot(_selectedID);
				_updateInfoText();
			}
		}
		else if (FlxG.mouse.justMoved && _selectedMugshot.visible)
		{
			FlxG.mouse.getWorldPosition(_selectedMugshot.camera, _point);
			_selectedMugshot.setPosition((_point.x - _startMousePos.x) + _startMugshotPos.x, (_point.y - _startMousePos.y) + _startMugshotPos.y);
		}
	...
	var _selectedMugshot:FlxSprite;
	var _startMousePos:FlxPoint = FlxPoint.get();
	var _startMugshotPos:FlxPoint = FlxPoint.get();
	...
	function _showMessage(text:String, type:MessageType = NORMAL)
	{
		...
		FlxTween.tween(_message, {alpha: 0.00001}, 1, {startDelay: 2});
	}
